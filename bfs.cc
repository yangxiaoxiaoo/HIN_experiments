...
std::pair<std::vector<int>, float> plain_bfs(const graph_t& g, Query query);
std::pair<std::vector<int>, float> bfs_oracle(const graph_t& g, PrunedLandmarkLabeling<> &pll, Query query);
std::pair<std::vector<int>, float> bidirectional_prune_bfs(const graph_t& g, Query query);
std::pair<std::vector<int>, float> bidirectional_bfs(const graph_t& g, Query query);
std::pair<std::vector<int>, float> bidirectional_bfs_exact_oracle(\
                const graph_t& g, PrunedLandmarkLabeling<> &pll, Query query);
*/

//plain BFS approach without using distance oracle and bi-direcional search. 
pair<vector<int>, float> plain_bfs(const graph_t& g, Query query){
	float minWgt = MAX_WEIGHT; 
	assert(query.pattern.size()>2);//otherwise the problem becomes trivial.
	vector<int> result;
	if( (g.typeMap[query.src]!=query.pattern[0]) || (g.typeMap[query.tgt]!=query.pattern[query.pattern.size()-1]) ){
		cout<< "src or tgt node does not follow pattern!" << endl;
		pair<vector<int>, float> pResult =  make_pair(result, -1);
		return pResult;
	}
	...
	result.push_back(numPath);//important.second last node in the result is # paths.
...
//Iterative BFS with exact distance oracle.
pair<vector<int>, float> bfs_oracle(const graph_t& g, PrunedLandmarkLabeling<> &pll, Query query){
	//Note: please use the distance oracle here to quickly check two nodes are impossible to have paths following the pattern.
	float minWgt = MAX_WEIGHT; 
	assert(query.pattern.size()>2);//otherwise the problem becomes trivial.
	vector<int> result;
	if( (g.typeMap[query.src]!=query.pattern[0]) || (g.typeMap[query.tgt]!=query.pattern[query.pattern.size()-1]) ){
	...
	for(int i=1; i<query.pattern.size(); i++){//expand by one layer forward. 
		...
		for(vector<vector<int> >::iterator it=frontier.begin(); it!=frontier.end(); it++){//for each node on the frontier.
			...
			for(int j=0; j<g.degree[curNode]; j++){//expand the neighbors of the frontier nodes. 
				...
				if(i==query.pattern.size()-1){//for last frontier. 
					mem ++;
					if(neigh == query.tgt && prewgt+wgt < minWgt){
						minWgt = prewgt + wgt;
						path.push_back(neigh);
						result = path;
						path.pop_back();
...
//bidirecional BFS WITHOUT distance oracle
pair<vector<int>, float> bidirectional_bfs(const graph_t& g, Query query){
			...
			for(int k=0; k<backwardPaths.size(); k++){
				if(!contain_loop(forwardPaths[j], backwardPaths[k])){
					...
					if(wgt1[j]+wgt2[k] < minWgt){
						minWgt = wgt1[j]+wgt2[k];
						result = forwardPaths[j];
	...
	int mem = forward.size()+backward.size();
	result.push_back(mem);
	if(minWgt >= MAX_WEIGHT)
		minWgt = -1;
	return make_pair(result, minWgt);
}
//Bidirectional BFS and also use distance oracle. 
pair<vector<int>, float> bidirectional_bfs_exact_oracle(const graph_t& g, PrunedLandmarkLabeling<> &pll, Query query){
	//please use the distance oracle here to quickly check two nodes are impossible to have paths following the pattern. 
	assert(query.pattern.size()>2);//otherwise the problem becomes trivial.
	vector<int> result;
	if( (g.typeMap[query.src]!=query.pattern[0]) || (g.typeMap[query.tgt]!=query.pattern[query.pattern.size()-1]) ){
...
pair<vector<int>, float> bidirectional_prune_bfs_prophet(const graph_t& g, Query query){
	assert(query.pattern.size()>2);//otherwise the problem becomes trivial. 
	vector<int> result;
	if( (g.typeMap[query.src]!=query.pattern[0]) || (g.typeMap[query.tgt]!=query.pattern[query.pattern.size()-1]) ){
		cout<< "src or tgt node does not follow pattern!" << endl;
	...
	int mem = forward.size()+backward.size();
	result.push_back(mem);
//	result.push_back(pathNum);result.push_back(layers[midLayer-1].size()); result.push_back(forward.size()); 
//	result.push_back(backward.size()); result.push_back(maxProd);
	//note: # paths of forward/backward in the last three entities in result.
	return make_pair(result, minWgt);
}
...
pair<vector<int>, float> bidirectional_prune_bfs(const graph_t& g, Query query){
	assert(query.pattern.size()>2);//otherwise the problem becomes trivial. 
	vector<int> result;
	if( (g.typeMap[query.src]!=query.pattern[0]) || (g.typeMap[query.tgt]!=query.pattern[query.pattern.size()-1]) ){
		cout<< "src or tgt node does not follow pattern!" << endl;
